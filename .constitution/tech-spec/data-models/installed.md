...
Each `InstalledSkill` records the source string, the source kind, the install scope (`project` or `user`), the harnesses written, the skill format, and a per-file `sha256` list used by `update`. A user-scoped record is identified by name and scope. A project-scoped record is identified by name, scope, and project root. Two installs can share a name. Upsert re-sorts by name, scope, and project root.
