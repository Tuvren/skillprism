...
- Honors ADR-0003 (single crate — new code in `src/distribution/network.rs`), ADR-0004 (synchronous — `Command::status` is blocking), ADR-0005 (atomic writes — downloaded files written via the existing `Router::write` infrastructure).
...
`update` re-fetches the source, re-renders/re-copies, computes SHA-256 of each output file, and compares against the `files` array in the state record. Only files whose hash differs from the stored hash are written (atomically per ADR-0005). Strictly better than Vercel's whole-folder `cleanAndCreateDirectory` approach and implementable in skillprism because the build pipeline already has the content-equality infrastructure.
...
The constraints/strategy amendments and ADR-0008 land in the same PR as this spike, so DIST-I002 (network layer) can begin implementation immediately after this PR merges.
