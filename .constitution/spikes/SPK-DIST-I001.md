...
- **Primary path:** `simple-git` (a Node wrapper around the `git` binary) called with `['--depth', '1', '--branch', ref]` flags. The simple-git client configures `GIT_TERMINAL_PROMPT=0` (disable interactive password prompts) and `GIT_LFS_SKIP_SMUDGE=1` plus `filter.lfs.{required,smudge,clean,process}=false/empty/empty/empty` to avoid the `git-lfs filter-process: command not found` failure mode when git-lfs is not installed. See `src/git.ts:60-95`. Note: the per-clone `git config` calls for the LFS filter (the `config: [...]` block in `createGitClient`) are intentionally NOT adopted by skillprism's ADR-008 — they require per-clone config writes that complicate the temp-dir lifecycle (the `git config` writes to a config file inside the clone dir that the implementation would need to clean up alongside the temp dir). Setting `GIT_LFS_SKIP_SMUDGE=1` alone is sufficient for skillprism's scope: skills are plain text, never LFS-tracked, and the env-var approach avoids the cleanup complication.
...
- **Concurrency:** v1 does not support concurrent `add` calls in the same state directory. `flock` is out of scope (matches the pre-spike planning decision).
